...
可以发现，想要给`PHP`添加新函数，只需要用`PHP_FUNCTION`宏包装一下即可。`helloworld`最为简单，深得我心。目前`PHP 8.2.0`除了定义函数外，还需要在`ext/standard/basic_functions.stub.php`中加上`函数原型`，姑且叫它函数原型吧，就看它像`C`语言的函数原型。至于`PHP 8.2.0`之前的版本先不管，再次在文件末尾加上以下代码：
