...
func (c *Client) Set(name, value string) string {
	cmd := assemble([]string{
		"set",
		name,
		value,
...
由于本文只是为了简单说明 Go 实现 Redis 客户端的原理，仅仅实现了部分功能，代码也没有做任何优化。如果有兴趣，可以继续深入学习，文章就到这了。
