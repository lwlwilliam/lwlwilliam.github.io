...
另外，可以看到所有记录的 TTL 值是 3600000 秒，相当于 1000 小时，也就是每 1000 小时才查询一次根域名服务器列表。目前，世界上一共有十三组域名
服务器，从`A.ROOT-SERVERS.NET`一直到`M.ROOT-SERVERS.NET`。
...
第二段显示了`.com`域名的 13 条 NS 记录，同时返回的还有每一条记录对应的 IP 地址（**TODO: 这里暂时不明白，只看到一个 IP 啊**）。

第三段 DNS 服务器向这些顶级域名服务器发出查询请求，询问`github.com`的次级域名`NS 记录`。
...
上面结果显示，`facebook.github.io`的 CNAME 记录指向`github.map.fastly.net`。也就是说，用户查询`facebook.github.io`的时候，实际上返回
的是`github.map.fastly.net`的 IP 地址（**TODO:实际运行命令，跟以上结果不一致，暂时没找到 CNAME 的实例**）。这样的好处是，变更服务器 IP 地址
的时候，只要修改`github.map.fastly.net`这个域名就可以了，用户的`facebook.github.io`域名不用修改。
